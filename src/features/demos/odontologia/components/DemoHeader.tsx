...
export function DemoHeader() {
    ...
    const isActive = (path: string) => {
        if (path === DEMO_BASE) return location.pathname === DEMO_BASE || location.pathname === `${DEMO_BASE}/`;
        return location.pathname.startsWith(path);
    };

    const navLinks = [
        { path: DEMO_BASE, label: 'Inicio' },
        { path: `${DEMO_BASE}/servicios`, label: 'Servicios' },
        { path: `${DEMO_BASE}/credenciales`, label: 'Credenciales' },
        { path: `${DEMO_BASE}/contacto`, label: 'Contacto' },
    ];

    return (
        <header className="sticky top-[44px] z-50 bg-card/95 backdrop-blur-sm border-b border-border">
            <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
                <div className="flex items-center justify-between h-20">
                    {/* Logo */}
                    <Link to={DEMO_BASE} className="flex items-center gap-3 group">
                        <div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center">
                            <span className="text-xl text-primary-foreground">DR</span>
                        </div>
                        <div className="hidden sm:block">
                            <div className="text-lg font-semibold text-foreground">Dra. Valeria Romero</div>
                    ...
                    <div className="hidden lg:flex items-center gap-4">
                        {/* Language Selector (decorative in demo) */}
                ...
                {mobileMenuOpen && (
                    <div className="lg:hidden border-t border-border py-4 space-y-4">
                        ...
                        <div className="px-4 pt-4 border-t border-border space-y-3">
                            ...
                            <div className="flex gap-2 flex-wrap">
                                {languages.map((lang) => (
                                    <button
                                        ...
                                        className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${currentLang === lang.code
                            ...
                            <Link to={`${DEMO_BASE}/reservar`} onClick={() => setMobileMenuOpen(false)}>
